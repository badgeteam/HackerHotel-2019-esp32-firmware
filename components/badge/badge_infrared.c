...
#define NEC_BIT_MARGIN         50                          /*!< NEC parse margin time */
...
inline bool nec_check_in_range(int duration_ticks, int target_us, int margin_us)
{
    if(( NEC_ITEM_DURATION(duration_ticks) < (target_us + margin_us))
		...
		//ESP_LOGW(NEC_TAG, "RANGE CHECK FAIL %d, %d, %d",NEC_ITEM_DURATION(duration_ticks), target_us, margin_us);
        return false;
    }
}
...
{
	//ESP_LOGW(NEC_TAG, "nec_parse_items");
    int w_len = item_num;
    ...
    *addr = addr_t;
    *data = data_t;
	//ESP_LOGW(NEC_TAG, "parse result %d", i);
    return i;
}
    ...
    while(rb) {
        ...
        if(item) {
            ...
            while(1) {
                ...
                if(res > 0) {
                    offset += res + 1;
                    ESP_LOGW(NEC_TAG, "RMT RCV --- addr: 0x%04x cmd: 0x%04x", rmt_addr, rmt_cmd);
                } else {
					//ESP_LOGW(NEC_TAG, "RMT RCV --- break");
                    break;
                }
            }
			...
			ESP_LOGW(NEC_TAG, "RMT RCV --- no item");
            break;
        }
    }
#endif
	ESP_LOGW(NEC_TAG, "Stop.");
    vTaskDelete(NULL);
}
...
static void rmt_example_nec_tx_task()
{
    ...
    uint16_t cmd = 0x1337;
    uint16_t addr = 0x4242;
    int nec_tx_num = RMT_TX_DATA_NUM;
    for(;;) {
        ESP_LOGW(NEC_TAG, "RMT TX DATA");
...
void start_infrared_rx()
{
	ESP_LOGW(NEC_TAG, "Start RX.");
    xTaskCreate(rmt_example_nec_rx_task, "rmt_nec_rx_task", 2048, NULL, 10, NULL);
}

void start_infrared_tx()
{
	ESP_LOGW(NEC_TAG, "Start TX.");
